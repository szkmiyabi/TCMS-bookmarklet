javascript:(function(){
  ...
  nd.writeln('<title>Bukken-util</title>');
  nd.writeln('<style>');
  ...
  nd.writeln('}');
  nd.writeln('#srccode {');
  nd.writeln('height: 10em;');
  nd.writeln('width: 100%;');
  nd.writeln('}');
  nd.writeln('#hint {');
  nd.writeln('margin: 25px 0;');
  nd.writeln('background: #FBE6EF;');
  nd.writeln('padding: 15px;');
  nd.writeln('min-height: 90px;');
  nd.writeln('}');
  nd.writeln('</style>');
  nd.writeln('<script>');
  ...
  nd.writeln('ht().innerHTML = "必ず、マップ画像のimgタグの中に貼り付けてください。<br>href属性値の中の[name]は、中見出しのアンカー名と同じにしてください。バルーンの個所を「spot」という文字で表現し、その後ろに連番を付けてください。ここでは、半角英数字を使ってください。<br>alt属性値は、バルーンの個所を「スポット」という文字で表現し、その後ろに連番をつけてください。<br>style属性値の中で、マップ画像上の黄色バルーンの位置を数値で指定します。top: は、ものさしツールで測定した上からの位置座標をpxで、left: は左からの位置座標をpxで指定してください。ここでも半角英数字を使ってください。";');
  nd.writeln('}');
  nd.writeln('function do_blue_baloon() {');
  nd.writeln('sc().value = \'<a href="#[name]-spot1"><img src="/file/img/465889.png" alt="スポット1" style="position: absolute; top: 218px; left: 353px;"></a>\';');
  nd.writeln('ht().innerHTML = "必ず、マップ画像のimgタグの中に貼り付けてください。<br>href属性値の中の[name]は、中見出しのアンカー名と同じにしてください。バルーンの個所を「spot」という文字で表現し、その後ろに連番を付けてください。ここでは、半角英数字を使ってください。<br>alt属性値は、バルーンの個所を「スポット」という文字で表現し、その後ろに連番をつけてください。<br>style属性値の中で、マップ画像上の青色バルーンの位置を数値で指定します。top: は、ものさしツールで測定した上からの位置座標をpxで、left: は左からの位置座標をpxで指定してください。ここでも半角英数字を使ってください。";');
  nd.writeln('}');
  nd.writeln('function do_h3_anchor() {');
