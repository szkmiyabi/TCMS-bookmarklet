javascript:(function(){
	var uitype="newtab"; /* popup, newtab */
	var acttype="text-link"; /* text, text-link */
	function TCMSUtil() {
		d = document;
		url = location.href;
	...
	TCMSUtil.prototype = {
		...
		upload_img_path: function(type) {
			...
			if(type === "popup") alert(uploadpath + selection);
			else this.browse_new_tab(uploadpath + selection);
		},
		...
		click_in_tree_keywd(keywd) {
			var pat = new RegExp(keywd);
			var ul = d.getElementsByClassName("list").item(0);
			var ls = ul.getElementsByTagName("li");
			for(var i=0; i<ls.length; i++) {
				var li = ls.item(i);
				var atag = li.getElementsByTagName("a").item(0); 
				var sts = atag.textContent;
				if(pat.test(sts)) {
					atag.click();
					break;
				}
		...
		delete_in_tree_keywd(keywd) {
			var crpat = new RegExp(/\/cms\/site_maps\/frame/);
			if(crpat.test(url)) {
				this.click_in_tree_keywd(keywd);
			} else {
				var pr = d.getElementsByClassName("line").item(0);
				pr.getElementsByTagName("li").item(1).getElementsByTagName("button").item(0).click();				
			}
		}
	};
	...
	util.delete_in_tree_keywd("テキスト");
