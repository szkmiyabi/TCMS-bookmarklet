...
javascript:(function(){
	...
	TCMSAdminUtil.prototype = {
		do_cat_input_from_prompt: function() {
			var src = prompt("表示順:カテゴリ名 をペーストしてください");
			if(src == "") return;
			var tmp = src.split(":");
			if(tmp.length == 0 || tmp == null) return;
			var order = tmp[0];
			var catname = tmp[1];
			this.cat_add_name.value = catname;
			this.cat_add_disp_order.value = order;
		},
	...
	if(util.cr_cat_add_pat.test(util.url)) {
		if(prompt_flg) util.do_cat_input_from_prompt();
		util.do_click_cat_add_save_btn();
	}
})();
